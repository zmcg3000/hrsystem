...
function AddUpdateScreen({ route, navigation }) {
    ...
    useEffect(() => {
        if (isEditing) {
            console.log(`Fetching URL: https://b3e0-180-150-7-92.ngrok-free.app/people/${staffId}`);
            // Fetch staff member's data for editing
            fetch(`https://b3e0-180-150-7-92.ngrok-free.app/people/${staffId}`)
    ...
    const handleSave = () => {
        ...
        const url = `https://b3e0-180-150-7-92.ngrok-free.app/people${isEditing ? `/${staffId}` : ''}`;
